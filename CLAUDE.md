...
This repository contains reusable agentic workflow packs for Claude Code. Each workflow pack includes agents, slash commands, and optionally GitHub Actions that work together to accomplish specific tasks like code reviews, design reviews, and security scanning.

## Directory Structure

```
agent-workflows/
├── claude/                    # Workflow packs organized by platform
│   ├── general/               # Platform-agnostic workflows
│   │   ├── code-review/       # Code review workflow pack
│   │   └── security-review/   # Security scanning workflow pack
│   ├── mobile/                # Mobile platform workflows
│   │   └── agents/            # iOS developer agent
...
Agent files use YAML frontmatter to configure the agent:
...
Action files follow standard GitHub Actions YAML format and typically use `anthropics/claude-code-action@v1` with custom prompts and tool configurations.
...
- **Mobile MCP**: Device control for mobile workflows (iOS development)
...
### File Naming
- Agent files: `workflow-name.md` (e.g., `code-review.md`)
...
### Agent Configuration
- Use `model: opus` for complex review tasks requiring deep analysis
- Use `model: sonnet` for standard development tasks
- Include comprehensive tool lists appropriate to the task
- Write detailed descriptions with usage examples
...
1. Create a new directory under the appropriate platform (`general/`, `web/`, `mobile/`)
2. Add a `README.md` with concept overview, methodology, and resource links
3. Create `agents/` directory with agent markdown files
4. Create `commands/` directory with command markdown files
5. Optionally add `actions/` for GitHub Actions workflows
6. Update the root `README.md` to include the new workflow
7. If the workflow requires an MCP server, document it in `/mcp`

## Installation Pattern

Workflows are installed by copying assets to the user's Claude directory:

```bash
# Copy agents
cp -r workflow-name/agents/* ~/.claude/agents/

# Copy commands
cp -r workflow-name/commands/* ~/.claude/commands/
